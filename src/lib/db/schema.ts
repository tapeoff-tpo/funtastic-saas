...
export const purchaseRequestItems = pgTable(
  'purchase_request_items',
  {
    ...
    bulkPaymentDepositCny: numeric('bulk_payment_deposit_cny', { precision: 16, scale: 2 }).notNull().default('0'),
    bulkPaymentDepositKrw: numeric('bulk_payment_deposit_krw', { precision: 16, scale: 2 }).notNull().default('0'),
    bulkPaymentDepositPaidAt: date('bulk_payment_deposit_paid_at'),
    bulkPaymentDepositMemo: text('bulk_payment_deposit_memo'),
...
export const gptAccounts = pgTable(
  'gpt_accounts',
  {
    ...
    resetAvailableCount: integer('reset_available_count').notNull().default(0),
    sharedUse: boolean('shared_use').notNull().default(false),
    notes: text('notes'),
    sortOrder: integer('sort_order').notNull().default(0),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
...
export const dealEvents = pgTable(
  'deal_events',
  {
    ...
    searchBid: integer('search_bid'),
    recommendationBid: integer('recommendation_bid'),
...
export const sourcingItems = pgTable(
  'sourcing_items',
  {
    ...
    imageContentType: varchar('image_content_type', { length: 160 }),
    imageFileSize: integer('image_file_size'),
    imageFileData: customType<{ data: Buffer }>({
      dataType() {
        return 'bytea'
      },
    })('image_file_data'),
    ...
    domesticSalePrice: integer('domestic_sale_price'),
    ...
    passedNewProductId: uuid('passed_new_product_id'),
    memo: text('memo'),
    rawData: jsonb('raw_data').$type<Record<string, unknown>>(),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
