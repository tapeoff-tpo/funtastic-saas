    ...
    switch (marketplaceId) {
      ...
      case 'ownerclan':
        result = await new OwnerclanAdapter({
          vendor_id: credentials.vendor_id?.trim() ?? credentials.username?.trim() ?? credentials.seller_id?.trim() ?? '',
          vendor_password: credentials.vendor_password?.trim() ?? credentials.password?.trim() ?? credentials.api_key?.trim() ?? '',
        }).testConnection()
        break
      default:
        return {
