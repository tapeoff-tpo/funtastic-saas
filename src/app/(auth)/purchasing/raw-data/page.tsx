...
export default async function PurchasingRawDataPage() {
  ...
  const [inventoryState] = await db
    .select({ lastUpdatedAt: max(inventory.updatedAt) })
    .from(inventory)
    .where(eq(inventory.userId, workspaceUserId))
  ...
  return (
    <div className="space-y-4">
      ...
      <PurchasingRawDataUpload today={today} inventoryUpdatedDate={inventoryUpdatedDate} />
    </div>
  )
}
