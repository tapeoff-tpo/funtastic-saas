...
export interface OrderRow {
  ...
  claimRequestReasonHistory?: Array<{ reason: string; registeredAt: string }>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" onClick={() => setOpen(false)}>
          <div className="w-full max-w-md rounded-lg bg-white p-4 shadow-xl" onClick={(event) => event.stopPropagation()}>
            ...
            <ClaimStatusActions
              ...
              requestReasonHistory={requestReasonHistory}
            />
          </div>
        </div>
      )}
          ...
          <div className="flex items-center gap-1">
            {order.claimId && order.claimType && order.claimStatus ? (
              <ClaimActionDropdown
                ...
                requestReasonHistory={order.claimRequestReasonHistory ?? []}
              />
            ) : (
