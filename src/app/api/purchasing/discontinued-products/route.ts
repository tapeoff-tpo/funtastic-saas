...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const summary = {
      ...
      discontinuedSkuCount: parsed.actions.filter((action) => action.action === 'discontinued').length,
      restoredSkuCount: parsed.actions.filter((action) => action.action === 'active').length,
