/**
 * 카카오선물하기 marketplace adapter implementing MarketplaceAdapter.
 *
 * Uses KakaoAK API key authentication with a JSON REST API.
...
import { createKakaoGiftClient } from './client'
import { mapKakaoGiftStatus, mapKakaoGiftClaimStatus, mapKakaoGiftClaimType } from './status-map'
import { mapCarrierCode } from '@/lib/shipping/carrier-codes'
import type {
  KakaoGiftOrder,
  KakaoGiftOrderResponse,
  KakaoGiftClaimResponse,
  KakaoGiftProduct,
  KakaoGiftProductResponse,
} from './types'

const KAKAO_GIFT_CONFIG: MarketplaceConfig = {
  id: 'kakao-gift',
  name: '카카오선물하기',
  authType: 'api_key',
...
export class KakaoGiftAdapter implements MarketplaceAdapter {
  readonly config = KAKAO_GIFT_CONFIG

  private readonly client: ReturnType<typeof createKakaoGiftClient>

  constructor(credentials: { api_key: string; store_id: string }) {
    this.client = createKakaoGiftClient(credentials.api_key)
  }
  ...
  async getOrders(since: Date): Promise<NormalizedOrder[]> {
    try {
      ...
      }).json<KakaoGiftOrderResponse>()

      return (response.orders ?? []).map((order) => this.normalizeOrder(order))
    } catch (error) {
      if (error instanceof MarketplaceApiError) throw error
      if (error instanceof Error && (error.message.includes('401') || error.message.includes('403'))) {
        throw new MarketplaceAuthError('kakao-gift', 'API key authentication failed')
      }
      throw new MarketplaceApiError('kakao-gift', 500, error instanceof Error ? error.message : 'Unknown error')
    }
  }
  ...
  async getClaimsOrders(since: Date): Promise<NormalizedClaim[]> {
    try {
      ...
      }).json<KakaoGiftClaimResponse>()

      return (response.claims ?? []).map((claim) => ({
        marketplaceClaimId: claim.claim_id,
        marketplaceId: 'kakao-gift' as const,
        marketplaceOrderId: claim.order_no,
        claimType: mapKakaoGiftClaimType(claim.claim_type),
        claimStatus: mapKakaoGiftClaimStatus(claim.claim_status),
  ...
  async uploadInvoice(orderId: string, invoice: InvoiceData): Promise<{ success: boolean; error?: string }> {
    try {
      const carrierCode = mapCarrierCode('kakao-gift', invoice.carrierId)
  ...
  private normalizeOrder(order: KakaoGiftOrder): NormalizedOrder {
    ...
    return {
      ...
      marketplaceId: 'kakao-gift',
      marketplaceStatus: order.status,
      status: mapKakaoGiftStatus(order.status),
  ...
  private normalizeProduct(product: KakaoGiftProduct): NormalizedProduct {
    return {
