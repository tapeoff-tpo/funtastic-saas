...
export const metadata: Metadata = { title: '발주 품목' }
          ...
          <tbody>
            {items.length === 0 ? (
              ...
              <tr key={item.id} className="border-b hover:bg-muted/40">
                {ESA009M_HEADERS.map((header) => (
                  <td key={header} className={`max-w-80 px-3 py-2 align-top ${header === '품목명' ? 'whitespace-normal' : 'whitespace-nowrap'}`} title={item.data[header] ?? undefined}>
                    {item.data[header] || '-'}
                  </td>
                ))}
