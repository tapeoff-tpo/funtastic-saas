CREATE TABLE IF NOT EXISTS public.channel_sales_batches (
...
CREATE TABLE IF NOT EXISTS public.channel_sales_lines (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  batch_id uuid NOT NULL REFERENCES public.channel_sales_batches(id) ON DELETE CASCADE,
  user_id uuid NOT NULL,
  channel varchar(20) NOT NULL CHECK (channel IN ('rocket', 'bulk')),
...
CREATE INDEX IF NOT EXISTS channel_sales_batches_user_created_idx
  ON public.channel_sales_batches(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS channel_sales_lines_user_date_channel_idx
  ON public.channel_sales_lines(user_id, occurred_on, channel);
CREATE INDEX IF NOT EXISTS channel_sales_lines_user_sku_date_idx
  ON public.channel_sales_lines(user_id, source_sku, occurred_on);

ALTER TABLE public.channel_sales_batches ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.channel_sales_lines ENABLE ROW LEVEL SECURITY;
