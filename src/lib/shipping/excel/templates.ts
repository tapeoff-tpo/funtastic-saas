...
export const AVAILABLE_ORDER_FIELDS: OrderFieldDef[] = [
  ...
  { field: 'collectedAtDateTime', label: '수집일시(YYYY-MM-DD HH:MM)' },
...
export const BUILT_IN_CARRIER_TEMPLATES: BuiltInCarrierTemplateDef[] = [
  ...
  },
  {
    ...
    columns: [
      ...
      { header: '수집 상품명', field: 'collectedProductName', width: 34, required: false },
      { header: '수집 옵션', field: 'collectedOption', width: 24, required: false },
      { header: '주문수량', field: 'collectedQuantity', width: 10, required: false },
      { header: '배송비', field: 'salesShippingFee', width: 12, required: false },
      { header: '판매가', field: 'salesUnitPrice', width: 12, required: false },
      { header: '판매가x수량', field: 'salesTotalAmount', width: 15, required: false },
      { header: '판매자할인금액+수수료', field: 'salesDiscountAndFee', width: 22, required: false },
      { header: '결제금액', field: 'salesPaymentAmount', width: 14, required: false },
      { header: '최종결제금액', field: 'salesFinalPaymentAmount', width: 16, required: false },
      { header: '결제금액 수수료', field: 'salesPaymentFee', width: 17, required: false },
      { header: '순이익액', field: 'salesProfit', width: 14, required: false },
      { header: '택배사', field: 'carrierName', width: 14, required: false },
      { header: '송장번호', field: 'trackingNumber', width: 18, required: false },
      { header: '수집일자', field: 'collectedAt', width: 13, required: false },
      { header: '출고완료일자', field: 'shippedAt', width: 15, required: false },
      { header: '내부주문번호', field: 'internalNo', width: 18, required: true },
      { header: '내부상품코드', field: 'productCode', width: 18, required: true },
      { header: '확정상품명', field: 'productName', width: 34, required: false },
      { header: '확정옵션', field: 'optionText', width: 26, required: false },
      { header: '실 출고수량', field: 'quantity', width: 12, required: false },
      { header: '택배박스 사이즈', field: 'packaging', width: 18, required: false },
    ],
  },
  {
    id: 'builtin:short-meeting',
    carrierId: null,
    name: '4숏미팅',
    isDefault: true,
    columns: [
      ...
      { header: '확정 옵션명', field: 'optionText', width: 24, required: false },
      { header: '주문수량', field: 'collectedQuantity', width: 10, required: false },
      { header: '현재고', field: 'stock', width: 10, required: false },
      { header: '판매가', field: 'salesUnitPrice', width: 12, required: false },
      { header: '결제금액', field: 'salesPaymentAmount', width: 14, required: false },
      { header: '쇼핑몰 상품코드', field: 'marketplaceItemId', width: 18, required: false },
      { header: '수집 상품명', field: 'collectedProductName', width: 34, required: false },
      { header: '주문번호(쇼핑몰)', field: 'marketplaceOrderId', width: 22, required: false },
      { header: '수집 옵션', field: 'collectedOption', width: 24, required: false },
      { header: '실 출고수량', field: 'quantity', width: 12, required: false },
      { header: '판매가x수량', field: 'salesTotalAmount', width: 14, required: false },
      { header: '최종결제금액', field: 'salesFinalPaymentAmount', width: 16, required: false },
      { header: '수집일시(YYYY-MM-DD HH:MM)', field: 'collectedAtDateTime', width: 24, required: false },
      { header: '출고완료일자', field: 'shippedAt', width: 14, required: false },
      { header: '내부 주문번호', field: 'internalNo', width: 16, required: true },
      { header: '주문자명', field: 'buyerName', width: 16, required: false },
      { header: 'Location', field: 'location', width: 14, required: false },
    ],
  },
]
