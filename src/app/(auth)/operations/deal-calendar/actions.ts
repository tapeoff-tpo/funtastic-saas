...
export async function createDealEventAction(formData: FormData) { const userId = await workspaceId(); if (!userId) return; await createDealEvent({ userId, platform: String(formData.get('platform') || 'kakao'), dealType: String(formData.get('dealType')), title: String(formData.get('title')), productCode: String(formData.get('productCode')) || null, dealPrice: Number(formData.get('dealPrice')) || 0, startsOn: String(formData.get('startsOn')), endsOn: String(formData.get('endsOn')), status: 'draft', dailyCapacity: 500, stock: 500 }); revalidatePath('/operations/deal-calendar') }
